"""
Completed solutions for the first set of basic list exercises featured within
Google's Python course.

Usage:
    python 03_list_a.py
"""


def match_ends(words):
    """
    Returns the count of words (contained within a list) that have matching
    start and end characters.
    """

    count = 0

    for word in words:
        if len(word) > 1 and word[0] == word[-1]:
            count += 1

    return count


def front_x(words):
    """
    Returns a list of words sorted in ascending alphabetical order, with the
    exception that words starting with the 'x' character will be placed at the
    start of the list.
    """

    x_words = []
    non_x_words = []

    for word in sorted(words):
        if word[0] == "x":
            x_words.append(word)
        else:
            non_x_words.append(word)

    return x_words + non_x_words


def sort_last(tuples):
    """
    Returns a list of tuples sorted based on the last item (element) contained
    within the tuple.
    """

    return sorted(tuples, key=lambda t: t[-1])


def test(got, expected):
    """
    Outputs (prints) the test case result.
    """

    if got == expected:
        prefix = " OK "
    else:
        prefix = "  X "

    print "%s Got: %s, Expected: %s" % (prefix, repr(got), repr(expected))


def main():
    """
    Performs the test cases.
    """

    print "Function: match_ends"
    test(match_ends(["aba", "xyz", "aa", "x", "bbb"]), 3)
    test(match_ends(["", "x", "xy", "xyx", "xx"]), 2)
    test(match_ends(["aaa", "be", "abc", "hello"]), 1)

    print "\nFunction: front_x"
    test(front_x(["bbb", "ccc", "axx", "xzz", "xaa"]), ["xaa", "xzz", "axx", "bbb", "ccc"])
    test(front_x(["ccc", "bbb", "aaa", "xcc", "xaa"]), ["xaa", "xcc", "aaa", "bbb", "ccc"])
    test(front_x(["mix", "xyz", "apple", "xanadu", "aardvark"]), ["xanadu", "xyz", "aardvark", "apple", "mix"])

    print "\nFunction: sort_last"
    test(sort_last([(1, 3), (3, 2), (2, 1)]), [(2, 1), (3, 2), (1, 3)])
    test(sort_last([(2, 3), (1, 2), (3, 1)]), [(3, 1), (1, 2), (2, 3)])
    test(sort_last([(1, 7), (1, 3), (3, 4, 5), (2, 2)]), [(2, 2), (1, 3), (3, 4, 5), (1, 7)])


if __name__ == "__main__":
    main()